...
- docs(qa): Nacht-Review + Baseline-Notizen zum UI-Testlauf vom 24.08.
- fix(beta): BUG-0081 Nacht-Gate kappt die Ladung, BUG-0083 SoC-Nähe-Guard, BUG-0084 Helper-Clamp
- fix(wizard): Vorzeichen-Semantik am Akku-Leistungssensor — Labels folgen der Feedback-Achse
- fix(controller): Story 9.7b — Richtungs-Scope + Sustained-Relief-Release + Resume-Clear für die Haus-Eskalation (#9)
- docs(qa): Antwort-Sektion im UI-Testbericht — alle 7 Befunde umgesetzt (Commit-Tabelle)
- chore(tests): mypy-Fix am UI-02-Pin (cast auf app statt app.state)
- fix(wizard): UI-06 — Anzeigename auch im Erst-Setup, vorbefüllt aus dem Registry-Gerätenamen
- fix(anlage): UI-05 — Trucki-Klasse-output_only heißt wieder Akku, nicht „Wechselrichter (PV)"
- fix(ui): UI-04+UI-07 — Diagnose-Zyklen scrollen im eigenen Container, Touch-Ziele auf 44 px
- fix(wizard): UI-01 — Auto-Suggests laufen device-gescopet, kein Mischsatz und keine Fremd-Topologie mehr
- fix(funktionstest): UI-03 — Zielhardware-Karte zeigt im gescopten Add-Test nur die getestete Einheit
- fix(funktionstest): UI-02 — Entlade-Leg läuft auch nach Status-Stale-Recovery (fiel vorher still aus)
- docs(vorlage): Review-Härtung 2026-08-24 — 17 Funde (F1–F17, R1#4/#9) mit Commit-Tabelle
- perf(setup): F15 — ein Entity-Registry-Fetch pro /setup/entities statt zwei serieller Phasen
- fix(live): F5 — Eigenmodus-Handlungszeile unterscheidet die Latch-Quelle (Verhalten → Sensor-Check zuerst)
- fix(live): F4+R1#4 — Eigenmodus-Handlungszeile gewinnt auf open_loop und ist für Screenreader erreichbar
- fix(funktionstest): F14 — 5-Minuten-Button echt einmalig und nur wo er helfen kann
- fix(diagnose): F9 — Glossar-Eintrag „dispatched" rendert „Übernommen" statt des rohen Tokens
- fix(settings): F8+F13 — „automatisch gemessen" nur bei aktivem Auto-Takt, tp_s füllt den „antwortet"-Satz nicht mehr
- fix(live): F6+F7 — Mehrsystem-Erklär-Zeile zählt nur echte Akkus und behauptet keine starre Reihenfolge mehr
- fix(live): F2+F11+F16 — Schonfrist-Zusage nur wenn einlösbar, Reset räumt Trip-Stempel, Dauer statt Uhrzeit
- fix(wizard): F12+F17+R1#9 — Geräte-Gruppierung über die stabile device_id, auch bei Ein-Geräte-Setups
- fix(wizard): F3+F10 — Disabled-Hinweis auch im Empty-State, als ruhige Disclosure statt Domain-Rauschen
- fix(funktionstest): F1 — A.6 verlängert die Wartezeit jetzt wirklich (Timing-Replace + Wert-Match-Polling)
- docs(vorlage): ②-Welle — Umsetzungs-Tabelle (B, A.1–A.6) + Gate-Stand 2026-08-22
- chore(backend): ruff format auf den Dateien der Bug-Wellen 2026-08-20/22 (nur Formatierung)
- feat(wizard): A.2 — DevicePicker: Entities nach HA-Gerät gruppiert, Geschwister-Slots bevorzugen das Gerät des Lade-Eingangs
- feat(live): A.5 — Mehrsystem-Erklär-Zeile im Akku-Drill-down (Pool-Regel ehrlich erklärt)
- feat(wizard): A.1 — weiche Selbstregler-Warnung am Steuer-Aktor-Pick (BUG-0047-Familie)
- feat(wizard): A.6 — „Gerät braucht länger?": Funktionstest einmalig mit 5-Minuten-Fenster
- feat(wizard): A.4 — in HA deaktivierte Entities im Picker sichtbar (BUG-0032)
- feat(ux): Erklär-Pass — Pause-Ursache, Eigenmodus-Handlungszeile, θ/Takt getrennt, Status-Glossar
- feat(10.6): Prognose-Ebene pro Akku — Zuordnung über die HA-Geraetenamen
- fix(10.6): auto-erkannte PV-Quelle markiert den Clipping-Kandidaten
- feat(10.6): PV-Quelle pro Akku — direkter Sensor als Quelle Nr. 1, Auto-Detect ohne Onboarding-Frage, Tagesform pro Pool
- chore(ship-gate): Story 9.7 Slice B — Haus-Koordinations-Gate auf eingemessene Baseline (matrixcasper_0820 = 686 Starved-Ticks)
- fix(controller): Story 9.7 — Export-Realitäts-Gate für Haus-Akku-Clamp + Eskalations-Sperre (matrixcasper 8d6be943@08-20)
- docs(planning+qa): Epic-10-Stories 10.6–10.9 + Flotten-Review-Notes, Konzept-R8 Feld-Befunde Günstig-Laden-Test, Clipping-Konzept, Wizard-Führungs-Vorlage, QA-UI-Smoketest
- feat(epic-14): Stories 14.1/14.2 Clipping-Outlook + Schatten; fix(house-model): BUG-0077 Forecast-Plausibilitäts-Bänder + BUG-0078 foreign-source_sig-why
- fix(persistence): CI-Exit-Hang — Verbindung auch bei Nicht-OperationalError schließen
- fix(house_model): BUG-0054 — partieller Fold + Profil-Reset gegen die Lastprofil-Vergiftung
- chore(claude): /story-issue-Command — Test-Auftrag-Issues aus Story/Bug-Fix generieren, inkl. Anreicherung von Beta-Ingest-Issues
- fix(controller): BUG-0025 — richtungs-partielle Effektivitäts-Evaluation für einseitige Sensoren
- fix(persistence): BUG-0030/0068 Slice 1 — DB-Härtung unter Schreiblast
- chore(github): Test-Auftrag-Issue-Template für den UI-/Feld-Verifikations-Workflow
- test: mypy-strict-Fixups für die Bug-Welle (dispatch-Monkeypatch auf dispatcher-Modul, Controller-Return-Typ)
- fix(controller): BUG-0072 — Stunden-Deckel für den 9.6-Foreign-Write-Re-Assert
- fix(controller): 9.3-Anti-Flattern — Eskalations-Fenster eskaliert statt fix 300 s (BUG-0071-Dwell-Muster)
- fix(controller): BUG-0074 — MULTI-Nächte erzählen ehrlich (min/max_soc + Rail statt "Schritt zu klein")
- fix(controller): BUG-0075 — Readback-Demote feuert jetzt auch auf Mismatch-Stürme
- fix(executor): BUG-0073 — Entity-Range-Guard + Attempt-Epoche gegen ServiceCallError-Stürme
- fix(controller): SoC-Cap-Stop-Retry-Deadlock — Laden am leeren Akku wurde ausgehungert

## 0.1.181 — 2026-08-19

- feat(wizard): Sensor-Plausibilitäts-Schicht + Funktionstest beide Richtungen (Welle-2 Bausteine A+B)
- fix(effectiveness): Welle-2-Härtung — Resume-Deadlock (BUG-0069), feedback_implausible-Konsum + Sensor-Hero (BUG-0070), eskalierender Auto-Resume-Dwell (BUG-0071)
- fix(ci): frontend-builder-Stage auf --platform=$BUILDPLATFORM pinnen — npm ci deadlockte unter QEMU-arm64 (6-h-Job-Hang nach Cache-Verfall der 17-Tage-Pause)
- fix(beta): BUG-0064 SoC-Idle-Freshness-Kredit + BUG-0065 Richtungs-Effektivität & Verhaltens-Fremdregler-Latch (rabekatze F-1–F-3, observability-first)

## 0.1.180 — 2026-07-31

- fix(review): 9.6-Review-Fix-Welle 2026-07-31 — Latch-Slot-Bindung, Re-Assert-Commit-Fenster, Autorschafts-Härtung, fail-soft own_ha_user_id (+Doku-Nachträge 9.4/9.5/9.6)
- feat(9.6): Fremd-Write-Detektor auf Setpoint-Numbers — Autorschaft, Event-Re-Assert, ehrliche Multi-Akku-Anzeige, Diag-Setpoint-History

## 0.1.179 — 2026-07-29

- feat(wizard): Marken-Picker „Welchen Akku hast du?" — Recognition statt Topologie-Taxonomie
- docs(planning): Epic 11 ausformuliert (Preis-Naht, ehrlicher Euro, Schatten-Berater) + 3 Story-Specs + Epic-11-Context; QA-Tagesanalyse Eigeninstanz Multi-Pool 2026-07-27
- feat(10.11): Haus-Modell-Ehrlichkeit — Stale-Toleranz kausal gebunden, grid_only-Gate + Profil-Reset, DC-PV-Korrektur (output_only), battery_model_for-/pv_bias-Naht, Feedback-Implausibilitäts-Detektor
- fix(review): Review-Fix-Welle 2026-07-24 — HA-WS-Reject-Handling, Reconnect-Hydrate global, RFC-7807-Catch-all, SoC-Plausibilitäts-Guard, Funktionstest-Idle-Härtung
- feat(10.10): Energie-Slot-Ledger — 15-min-Wh-Wahrheit als energy_slots-Tabelle
- feat(ui): Live-Echo-/Klartext-Pass — formatMeasured-Single-Source, Dach-PV als Info-Baustein, Haus-Akku-Live-Vorschau, ehrliche Hero-Koordinationszeile
