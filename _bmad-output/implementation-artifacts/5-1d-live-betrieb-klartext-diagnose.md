...
- [ ] [Review][Decision] **MULTI-Mode Reason-Verhalten — wer „gewinnt" wenn Speicher und Drossel beide Reasons setzen?** — `_dispatch_by_mode` (controller.py:564) reset `_last_policy_noop_reason = None` einmal vor Dispatch. `_policy_multi` ruft (Spec) Speicher zuerst und Drossel als Fallback bei Cap+Einspeisung. Beide setzen Reasons → letzter writer gewinnt. Der User sieht im Cycle-Header nur einen Grund. Optionen: (a) Last-write-wins akzeptieren (zeigt warum die FINALE Branch nicht regelte) — current behavior; (b) First-write-wins (zeigt die ursprünglich blockierende Branch); (c) Concat zu `multi: speicher=…, drossel=…` mit `;`-Trenner. Risiko `(a)`: bei Speicher-Cap+Drossel-Deadband zeigt UI „Im Toleranzbereich" — verschluckt das eigentliche „Akku voll"-Signal.
- [ ] [Review][Decision] **`_effective_value_w` für non-grid_meter Rolle: `None` (Spec AC 14) oder raw value (aktueller Code)?** [backend/src/solalex/api/routes/control.py:247-248] — Spec sagt explizit „bei anderen Rollen `None` und der `state`-Wert ist die Wahrheit". Code returnt aber den raw `value` für alle non-grid_meter-Rollen. Test wurde an Code angepasst (`test_entity_snapshot_non_grid_meter_passes_state_through` asserts `effective_value_w == 800.0`). Optionen: (a) Code an Spec anpassen (return `None`) und Test umdrehen; (b) Spec an Code anpassen (raw-passthrough für alle Rollen, dokumentieren als „grid_meter ist die einzige Rolle, wo Sign-Invert greift, aber `effective_value_w` ist immer der UI-fertige Wert"). Frontend liest `effective_value_w` derzeit für Grid-Tile — wenn (b), kann Frontend für alle Tiles auf `effective_value_w` umgestellt werden (DRY).
- [ ] [Review][Decision] **`StateSnapshot.ha_ws_connected` defaults to `True` — Pre-Connect-Window zeigt fälschlich „Verbunden"** [backend/src/solalex/state_cache.py:818] — Code-Kommentar rechtfertigt das mit „Tests / dev without HA-WS supervisor". Aber Production-Risiko: wenn der Reconnect-Hook (main.py-Verkabelung) je bricht, zeigt UI dauerhaft „Verbunden" obwohl gar keine Verbindung besteht — silent failure. Optionen: (a) Default auf `False` umstellen, Test-Fixtures explizit auf `True` setzen wenn nötig; (b) `Optional[bool]` mit `None`=„unbekannt, wir warten auf erste Transition" → Frontend zeigt „Verbindung wird geprüft …"; (c) wie aktuell, plus startup-Lifespan-Hook im `main.py`, der explizit `False` setzt bevor der Reconnect-Loop startet.
