...
1. **Settings-Route versteckt erreichbar nach Pre-Disclaimer, voll nutzbar post-Commissioning (FR22, FR20):** `Given` der User ist commissioned (`devices.every(d => d.commissioned_at !== null)` ist `true`), `When` er `#/settings` aufruft, `Then` rendert `App.svelte` `<Settings />` ohne Redirect, **And** der `evaluateGate`-Decision-Tree liefert `{kind: 'stay'}` für `currentRoute === '/settings'` bei commissioned-State, **And** `'/settings'` ist in `VALID_ROUTES` aufgenommen (sonst Hash-Reset auf `/`), **And** `'/settings'` ist **nicht** in `WIZARD_ROUTES` (sonst würde 2.3a-Gate einen commissioned User auf `#/running` zurückwerfen — Test: `evaluateGate({devices: [commissioned], currentRoute: '/settings', preAccepted: true, allowAutoForward: false}) === {kind: 'stay'}`), **And** ein User ohne akzeptierten Pre-Disclaimer wird auf `#/disclaimer` gegated. **Decision 2026-04-25:** Ein `preAccepted` User darf die versteckte Route auch vor Commissioning öffnen; `Settings.svelte` zeigt dann einen eigenen "Noch nicht aktiviert"-Hinweis statt Akku-Form oder drossel-only-Hinweis.

2. **Settings-UI rendert Akku-Konfiguration (FR22, FR20):** `Given` `#/settings` ist erreichbar, `When` `Settings.svelte` rendert, `Then` lädt es per `client.getDevices()` die aktuelle Devices-Liste und extrahiert das commissioned `wr_charge`-Device (falls vorhanden); aus dessen `config_json` werden `min_soc`, `max_soc`, `night_discharge_enabled`, `night_start`, `night_end` als Initial-Werte der Form-Felder befüllt (Defaults `15`, `95`, `true`, `"20:00"`, `"06:00"` bei nicht-existenten Keys). **And** wenn Devices noch nicht commissioned sind oder noch keine Devices existieren, zeigt die Seite einen deutschen "Noch nicht aktiviert"-Hinweis. **And** wenn alle Devices commissioned sind, aber **kein** `wr_charge`-Device existiert (drossel-only-Setup ohne Akku), zeigt die Seite einen deutschen Hinweis-Text „Dieses Setup hat keinen Akku. Min-/Max-SoC und Nacht-Entlade sind hier nicht konfigurierbar." und blendet das Form-Feld-Card aus.

3. **Validation Min/Max-SoC Bounds (Epic-AC 2):** `Given` der User editiert die Min-/Max-SoC-Felder, `When` er einen Wert setzt der `min_soc + 10 ≥ max_soc` macht, `Then` zeigt die UI vor dem Save eine **Inline-Error-Zeile** unter der Card mit dem Klartext „Max-SoC muss mehr als 10 % über Min-SoC liegen" und der Save-Button ist disabled. **And** im PATCH-Backend erzwingen die Field-Bounds `min_soc <= 40` und `max_soc >= 51` bereits mindestens 11 Prozentpunkte Abstand; ein field-valider Gap-Verstoß ist dort unmöglich. Direkte API-Calls außerhalb dieser Bounds antworten mit `422 Unprocessable Entity`. Die gemeinsame `_validate_soc_gap`-Logik bleibt für Symmetrie mit `HardwareConfigRequest` bestehen, ist im PATCH-Pfad aber redundant. (Decision 2026-04-25: Story/Test auf diese Bounds korrigieren, nicht Schema lockern.)
...
19. **Akku-Tile-„Kein Akku"-Hinweis (Epic-AC 1, drossel-only-Setup):** `Given` der User hat ein WR-only-Setup commissioned, `When` `Settings.svelte` lädt, `Then` zeigt es eine **deutsche** Hinweis-Karte (kein Error-Block, kein Modal) mit dem Titel „Kein Akku konfiguriert" und dem Body-Text „Dieses Setup hat keinen Akku. Min-/Max-SoC und Nacht-Entlade sind hier nicht konfigurierbar. Wenn du später einen Akku hinzufügst, nutzt du den Reset-Flow und durchläufst den Setup-Wizard erneut." **And** ein Settings-Save-Button wird **nicht** gerendert. **And** bei uncommissioned Devices oder leerer Device-Liste zeigt Settings stattdessen „Noch nicht aktiviert" (nicht „Kein Akku"). **Begründung:** Story 3.6 covered explizit nur Akku-Bounds; Hardware-Identität-Edits sind out-of-scope.
...
24. **Frontend — Routing & Gate-Update:** `Given` die neue Route, `When` `App.svelte` und `lib/gate.ts` angepasst werden, `Then`:
    ...
    - `evaluateGate` returniert für `'/settings'` bei `devices.length === 0 && preAccepted` ebenfalls `{kind: 'stay'}` (User landet im „Noch nicht aktiviert"-Hinweis — kein impliziter Wizard-Kick).
...
25. **Tests Backend (Pytest):** Neue Test-Datei + Erweiterungen:
    - `backend/tests/unit/test_battery_config_route.py` (neu):
      ...
      - `test_patch_rejects_min_soc_above_patch_field_bound` (AC 3) — `min_soc=80, max_soc=85` → 422 wegen PATCH-Field-Bound; Gap-Regel ist unter field-validen PATCH-Werten redundant.
