...
- [x] [Review][Patch] **P1 [HIGH] — `activeRateLimit` zeigt `Math.max`, Label sagt „Nächster Write"** [`frontend/src/routes/Running.svelte`] — AC 5 (Blind+Edge). Fix angewendet: `Math.max` → `Math.min`. Regression-Test `surfaces the soonest cooldown (Math.min) when multiple devices are rate-limited` (zwei Devices 5 s / 55 s → „Nächster Write in 5 s").
- [x] [Review][Patch] **P2 [HIGH] — Target-Limit-Serie unsichtbar & Clock-Skew-Misalignment** [`frontend/src/routes/Running.svelte`] — AC 2 (Blind+Edge). Fix angewendet: neuer `targetBuffer` mit `Date.now()`-Stamping im `$effect`; Target-Wert wird aus dem jeweils neuesten Cycle (`recent_cycles[0].target_value_w`) bei jedem Poll-Tick ins Window gelegt. Series-Kante richtet sich jetzt mit grid/readback; NTP-Skew vs. Server-`ts` eliminiert; „letzter Dispatch-Target" bleibt als stehende Horizontale sichtbar.
- [x] [Review][Patch] **P3 [MEDIUM] — `int(remaining)` truncation → 1-s-Lücke im Countdown** [`backend/src/solalex/api/routes/control.py:163-165`] — AC 5 (Edge). Fix angewendet: `math.ceil(remaining)` statt `int(...)`.
- [x] [Review][Patch] **P4 [MEDIUM] — `RecentCycle.mode/source/readback_status` als `str` statt `Literal`** [`backend/src/solalex/api/schemas/control.py`, `frontend/src/lib/api/types.ts`] — AC 13 (Edge). Fix angewendet: neue Literal-Aliase `CycleMode`, `CycleSource`, `CycleReadbackStatus` im Pydantic-Schema; passende String-Unions `CycleSource`, `CycleReadbackStatus` + `ControlMode`-Reuse im TS. Zusätzlich Enforcement via Pydantic: DB-Werte außerhalb der erlaubten Strings liefern jetzt 500 statt stumm durchzureichen.
- [x] [Review][Patch] **P5 [MEDIUM] — Frontend-Tests testen weder 3-Serien noch 10-Einträge-Cap** [`frontend/src/routes/Running.test.ts`] — AC 14 (Auditor). Fix angewendet: vier neue Test-Cases (`caps the cycles list at ten entries`, `renders three chart series`, `shows the skeleton chart pulse and both empty-state hints`, plus P1-Regression). Vitest-Lauf: 53/53 passed (vorher 49).
- [x] [Review][Patch] **P6 [MEDIUM] — `source`-Badge-Farben nicht AC-konform** [`frontend/src/routes/Running.svelte`] — AC 4 (Auditor). Fix angewendet: `manual` nutzt `--color-neutral-muted` (neutrales Grau), `ha_automation` nutzt `--color-brand-ink` (Slate-900 Blau-Grau). Keine neuen Tokens — CSS Custom Properties aus `app.css` sind Single-Source.
- [x] [Review][Patch] **P7 [LOW] — `datetime.fromisoformat` ValueError wird geschluckt** [`backend/src/solalex/api/routes/control.py`] — CLAUDE.md Regel 5 (Blind). Fix angewendet: `_logger = get_logger(__name__)` + `_logger.exception("rate_limit_last_write_parse_failed", extra={"adapter_key": ..., "raw": ...})` im `except ValueError`-Zweig.
- [x] [Review][Patch] **P8 [LOW] — Controller mirrort Modus vor Cycle-Write, nicht am Cycle-Call-Site** [`backend/src/solalex/controller.py`] — AC 11 (Blind+Auditor). Fix angewendet: `update_mode`-Call aus `on_sensor_update` entfernt; jetzt genau dort platziert wo Cycle-Rows persistiert werden (`_record_noop_cycle` nach Insert/Commit, `_safe_dispatch` bei Success-Branch, `_write_failsafe_cycle` bei Persist). Bestehende Tests `test_controller_calls_update_mode_once_per_event` + `test_update_mode_not_called_when_test_in_progress` laufen weiter grün.
- [x] [Review][Patch] **P9 [LOW] — Cycle-`{#each}`-Key kann kollidieren** [`frontend/src/routes/Running.svelte`, `backend/src/solalex/api/schemas/control.py`, `frontend/src/lib/api/types.ts`] — AC 4 (Blind+Edge). Fix angewendet: `id: int` in `RecentCycle`-Pydantic + TS-Interface aufgenommen; `list_recent` liefert den Row-PK bereits. `{#each}`-Key jetzt `(cycle.id)`. Backend-Test `test_control_state_recent_cycles_returns_last_ten` asserted auf ID-Eindeutigkeit.
- [x] [Review][Patch] **P10 [LOW] — Idle-Override im Endpoint bei Heartbeat-Gap** [`backend/src/solalex/api/routes/control.py`] — AC 3, D1-Resolution. Fix angewendet: neue Helper-Funktion `_resolve_current_mode(cache_mode, recent_cycles, now)` mit 15-s-Grace-Window (`_IDLE_HEARTBEAT_WINDOW_S`). Leere Cycles-Liste **und** stale Cycles (> 15 s) führen zur Idle-Override. Neue Tests: `test_control_state_idle_override_when_heartbeat_stale` + `test_control_state_idle_override_when_no_cycles`. `test_control_state_current_mode_reflects_state_cache` erweitert um frischen Cycle-Seed damit Post-P8-Reality respektiert wird.

**CI-Gates (Post-Patch):** ruff clean, mypy strict (77 files) clean, pytest 138/138 passed (+2 vs. initial Review), ESLint clean, svelte-check 0 errors / 0 warnings (273 files), Vitest 53/53 passed (+4 vs. initial Review). Prettier-Warnungen sind pre-existing (14 Dateien inkl. Running.svelte — Baseline auf `0650862` identisch). Drift-Checks: 0 Treffer auf `openapi-typescript|date-fns|dayjs|moment|luxon` und 0 Treffer auf `WebSocket|EventSource` in Running.svelte.
...
- **2026-04-24 — Code Review + Patch-Batch angewendet.** `bmad-code-review` mit 3 parallelen Layern (Blind Hunter, Edge Case Hunter, Acceptance Auditor). Triage: 1 decision-needed (D1 → P10), 10 Patches, 1 Deferred auf Epic 4, 15 Dismissed. Alle 10 Patches in-Story gefixt: P1 (`Math.min`), P2 (target-Buffer mit `Date.now()`), P3 (`math.ceil` Rate-Limit), P4 (Literal-Types `RecentCycle`), P5 (4 neue Frontend-Tests), P6 (distinkte Source-Badges), P7 (Logger für Parse-Fehler), P8 (Mode-Mirror an Cycle-Call-Sites), P9 (`id` als `{#each}`-Key), P10 (15-s-Idle-Override im Endpoint). CI-Gates grün: Pytest 138/138, Vitest 53/53, mypy strict clean, ESLint + svelte-check clean. DF1 (Polling-Error-Surface) deferred auf Story 4.3.
