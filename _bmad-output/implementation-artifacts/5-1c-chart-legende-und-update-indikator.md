...
- **2026-04-25 — Zweite Scope-Erweiterung (Y-Achse + Legendenwerte):** Smoke-Test zeigte „aktuell zeigt zu wenig an" — kein Watt-Bezug am Chart, keine Live-Werte in der Legende. `LineChart.svelte` ergänzt um Y-Achse mit 3 Ticks (Min/Mitte/Max) in `formatWatts(v)`-Format (W bis 999, ab 1000 als kW mit 1 Nachkommastelle bis 9.9, ab 10 kW als ganze Zahl), dezente gestrichelte Grid-Lines an den Y-Ticks, Zero-Line hervorgehoben wenn der Wertebereich Null kreuzt (Bezug ↔ Einspeisung visuell sofort erkennbar). LEFT_PAD von 4 → 44 px für Y-Label-Spalte. `formatWatts`-Helper als `<script module>`-Export — von `Running.svelte`'s Legende reused, damit Zahlenformat auf Achse und in Legende identisch. `Running.svelte` Legende erweitert um `.legend-value`-Span pro Eintrag (zeigt aktuellen Wert via `series.data.at(-1).v`). 4 neue LineChart-Tests (Y-Labels in W, kW-Switch ab 1000, Zero-Line bei negativem Min, Zero-Line-Suppression bei rein positivem Range), 1 neuer Running-Test (`shows the latest value next to each legend entry`), 1 bestehender Test geschärft (`screen.getByText('310 W')` → `cycleList?.textContent.toContain('310 W')`, weil die Legende denselben Text jetzt auch zeigt). Frontend Vitest scope-Tests 28/28 grün, ESLint + svelte-check + Prettier sauber.
...
- [ ] [Review][Patch] **Chart-`gridBuffer` benutzt `entry.state` statt `entry.effective_value_w`** [frontend/src/routes/Running.svelte:221, 228] — Status-Tile (`status-tile-value`) zeigt `gridEntity.effective_value_w` (nach Story-2.5-Sign-Invert), aber die Linie unter dem Tile zeichnet das un-invertierte `entry.state`. Bei `invert_sign=True` (Alex' eigenes ESPHome-SML-Setup) zeigt der Chart das Vorzeichen umgekehrt zur Tile/Controller-Wirkung — sichtbarer Widerspruch direkt nebeneinander. Fix: in beiden Buffer-Updates `entry.effective_value_w ?? entry.state` lesen (mit Fallback für den nicht-grid-Fall, falls D3 entschieden ist).
- [ ] [Review][Patch] **`STALE_AFTER_MS = 5000` triggert false-stale bei normalen Polling-Hängern** [frontend/src/routes/Running.svelte:117] — Polling-Intervall ist 1 s; ein einzelner langsamer Request (Network-Hiccup ≥5 s = nicht ungewöhnlich auf HA-Mobile) flippt den Indikator stale, obwohl alles funktioniert. Vorschlag: 10 s ODER `3 * polling_interval`. Alternativ zusätzlich `polling.error`-Store subscriben, damit echte Errors den Indikator unabhängig vom Tick stale flippen.

### Deferred

- [x] [Review][Defer] **`visibilitychange`-Handler fehlt — 1 Hz Polling + 1 Hz Wall-Clock laufen auch im Hintergrund-Tab** [frontend/src/routes/Running.svelte:254-265] — Mobile Battery-Drain. Pre-existing-Pattern (5.1a Polling läuft auch im Hintergrund); eigene UX-Verbesserung später.
- [x] [Review][Defer] **`update-pulse`-CSS-Animation läuft auch wenn Polling stopped** [frontend/src/routes/Running.svelte:2208-2220] — Visueller Nit, irreführend wenn Tab im Hintergrund. Klein, aber außerhalb 5.1c-Scope; zusammen mit `visibilitychange`-Handler later.
