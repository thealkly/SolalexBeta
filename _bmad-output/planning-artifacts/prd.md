...
**Regelungs-Qualität:**
- Nulleinspeisung stabil auf Hoymiles (±5 W) und Anker (±30 W)
- Akku-Steuerung produktionsreif für Anker Solix, Marstek Venus 3E/D (Day-1, non-negotiable), Generic HA Entity
...
Reaktive Nulleinspeisung mit Adaptiver Regelungs-Strategie (Drossel / Speicher / Multi-Modus) · Akku-Pool-Abstraktion (v1 Gleichverteilung) · Hardware Day-1: Hoymiles/OpenDTU, Anker Solix, Marstek Venus, Shelly 3EM, Generic HA Entity · 1 WR + 1 SM + 1 Akku pro Instanz · Setup-Wizard mit Auto-Detection + Blueprint-Import + Funktionstest · Dashboard mit Euro-Wert · Diagnose-Tab · HA Add-on (Python 3.13 + FastAPI + Svelte + SQLite) · LemonSqueezy mit monatlicher Graceful-Degradation · Deutsch only.

**Geschäftsmodell offen (Beta-Entscheidung):** 30-Tage-Trial (Favorit) vs. Freemium 1-Gerät-Free/Multi-Pro.
**Bewusst NICHT im MVP:** MQTT Discovery · dynamische Tarife · Kaskaden · Multi-WR/Multi-Akku · Forecast · Englisch · Wallbox als gesteuerter Verbraucher.

### Growth — v1.5 (Monat 2)

MQTT Discovery für native HA-Entities · Dynamische Stromtarife mit Preis-Gates (Tibber, aWATTar, EPEX Spot, Nordpool) · Optionale 7-Tage-Baseline für Delta-KPI.

### Growth — v2 (Monat 3–4)

Kaskaden-Modell (Überschuss + Entlade mit Gates) · Multi-WR + Multi-Akku mit SoC-Balance · Solar-Forecast über SetpointProvider · **Haus-Profile** (5 One-Click-Presets: Eigenverbrauch max / Autarkie / Tarife / Komfort / Winter) · Englische UI · Failsafe-Modus · Erweiterte Device-Templates (Huawei, SMA, Growatt, Fronius).
...
- **Setup & Onboarding:** Drei-Pfade-Wizard (Hoymiles / Anker / Manuell) · Auto-Detection für OpenDTU, Shelly 3EM, Anker Solix, Marstek Venus · Live-Werte neben jedem Sensor · Blueprint-Import-Wizard mit Helfer-Erkennung · Funktionstest als Lern-Moment und Readback-Validierung · Landing-Page-Check für HA-OS/Supervised
...
- **EEPROM-/Schreibzyklen-Schutz:** Wechselrichter und Akku-BMS haben herstellerspezifische Schreibzyklus-Limits (z. B. Hoymiles ~1 Mio., undokumentiert bei Marstek/Anker). Solarbot implementiert **hardware-spezifisches Rate-Limiting** (Default ≤ 1 Schreibbefehl pro Device/Minute, per Device-Template überschreibbar) um Hardware-Verschleiß zu vermeiden.
...
| Hersteller-Cloud-API ändert sich/wird kostenpflichtig (Anker) | Generic-HA-Entity-Adapter als Fallback; Abhängigkeit auf Landing transparent kommuniziert |
...
| HA-Entity-Layer-Moat | Hardware-Diversität in Beta (Marstek, Anker, Hoymiles, SMA, Huawei, Growatt) demonstriert Hebelwirkung durch unterschiedliche HA-Integrationen |
...
| Adaptive Regelungs-Strategie | Zweigeteiltes Beta-Gate: Drossel-Modus stabil ±5 W bei Hoymiles (Benni/Nils-Setup) UND Speicher-Modus stabil ±30 W bei Anker/Marstek (Micha/Björn-Setup) plus explizit sauberer Modus-Wechsel „Akku-voll → Drossel" ohne Oszillation |
...
- **Device-Template-Schema.** Pro Hersteller/Modell ein JSON-Template mit: unterstützte Entity-Muster (Regex), Steuerung-Semantik (WR-Limit setzen / Akku-Setpoint / Charge/Discharge-Toggle), Default-Regelungs-Parameter (Deadband, Rate Limit, EEPROM-Schutz-Intervall), Latenz-Erwartung (für Timeout-Schwellen). Templates liegen in `/data/templates/` als versionierte JSON-Dateien.
...
- **Supported Day-1:** Hoymiles/OpenDTU, Anker Solix, Marstek Venus 3E/D, Shelly 3EM, Generic HA Entity (manueller Pfad).
...
- **Lizenz-Signatur:** Signierte Lizenzdatei in `/data/license.json`, kryptografisch verifiziert beim Start. Manipulations-Toleranz: keine.
...
- **Solo-Dev mit AI-Tooling:** Alex entwickelt mit Claude/Cursor. Test-First mit pytest, Komponenten-Isolation für adapter-spezifische Logik (ein Modul pro Device-Template).
...
2. **Balkon-Benni's Fundament** — zuverlässige Nulleinspeisung mit Hoymiles/Anker
...
**Core Journeys supported:** Marstek-Micha · Beta-Björn · Neugier-Nils · Alex Kly (Support)
...
- Akku-Pool-Abstraktion (v1 Gleichverteilung) für Marstek Venus + Anker Solix
- Hardware-Day-1: Hoymiles/OpenDTU, Anker Solix, Marstek Venus, Shelly 3EM, Generic HA Entity
- Setup-Wizard mit Auto-Detection, Live-Werten, Funktionstest
- Hardware-spezifische Default-Regelungs-Parameter via Device-Templates + E2E-Latenz-Messung
...
- Erweiterte Device-Templates (Huawei, SMA, Growatt, Fronius)
...
**Technical Risks:**
- **Marstek lokale API instabil oder undokumentiert** (Hauptrisiko des Projekts): Spike-Phase Woche 0 MUSS die Marstek-Kommunikation end-to-end validieren, bevor Wochen 1–6 beginnen. Firmware-Pin im Device-Template + versionstolerante Adapter. Fallback bei Scheitern: Generic HA Entity ohne Multi-Device — Marstek-Segment wird dann in v1.5 bedient statt Day-1.
- **Anker Cloud-API-Latenz verhindert Sekunden-Regelung:** Hardware-spezifische Deadband-Defaults (±30 W statt ±5 W), keine Sekunden-Zusage für Anker im Marketing — Anker bekommt eigene Toleranz-Kommunikation.
...
- **Lizenz-Signatur:** Kryptografisch verifiziert beim Start (`/data/license.json`).
...
- **Device-Template-Versionierung** mit Firmware-Pinning bei Marstek/Anker; versionstolerante JSON-Key-Adapter.
...
- **Modulare Architektur:** ein Python-Modul pro Device-Template (`adapters/marstek_venus.py`, `adapters/anker_solix.py`, etc.); Core-Regelung in `core/controller.py` hardware-agnostisch.
...
- **Hardware-Abdeckung:** Device-Template-System muss ≥ 10 weitere Hersteller (Huawei, SMA, Growatt, Fronius, Zendure, …) in v2–v3 erlauben ohne Core-Refactor.
...
- **Infrastruktur ab v1:** i18n-ready — alle UI-Strings extrahiert in `locales/de.json`, kein Hard-Coding.
