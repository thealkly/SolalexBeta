...
**Tiefen-Analyse mit Original-Nutzerstimmen, Begeisterungs-Features, Optimizer-Analyse und Solarbot-Differenzierung**
...
**Lehre für Solarbot:** Die Time-to-First-Erfolgserlebnis ist bei EVCC kurz. Nutzer berichten nach drei Tagen schon Begeisterung. Das ist die Latte. Wenn Solarbot 10 Minuten verspricht, müssen die ersten Sekunden danach schon Wow-Momente liefern — sonst klingt das wie Marketing-Übertreibung.
...
**Lehre für Solarbot:** Die automatische Phasenumschaltung im Sommer ist EVCCs Magie-Moment. Nutzer fühlen sich dem Hersteller technisch überlegen. Solarbot braucht eine ähnliche emotionale Magie — bei uns könnte das **die EEPROM-Schonung sichtbar machen** sein ("Solarbot hat heute 142 statt 14.000 Schreibvorgänge an deinen Wechselrichter gesendet — deine Hardware bedankt sich"). Magie entsteht, wenn der Nutzer etwas sieht, was er ohne uns nicht hätte.
...
**Lehre für Solarbot:** Migration ist ein massiver Wachstumstreiber. EVCC saugt Nutzer aus openWB ab, weil openWB komplexer ist und teurer. Solarbot muss eine analoge Migration aktiv anbieten — von Hersteller-Apps und vom eigenen Blueprint.
...
**Lehre für Solarbot:** Schlicht schlägt featurelastig. Das ist ein Punkt, an dem Solarbot mit dem v1.2-Design (Klarheit vor Features, Ergebnis vor Technik) bereits richtig liegt — aber er muss konsequent durchgehalten werden, gerade wenn die Roadmap wächst.
...
**Lehre für Solarbot:** Die EVCC-Templates sind ein Network-Effekt-Asset. Solarbot kann das nicht direkt kopieren — aber das Profile-Marketplace-Konzept aus der Roadmap (V1.x) ist die analoge Antwort: crowdsourced Wissen über Hardware-Profile, das mit jedem Nutzer wertvoller wird.
...
**Lehre für Solarbot:** EVCC hat einen sichtbaren "magischen Knopf", der eine komplexe Strategie auf einen Klick reduziert. Solarbot braucht solche Knöpfe — z. B. "Akku jetzt voll machen", "Jetzt nichts einspeisen", "Heute Abend Wärmepumpe vorheizen". Komplexität verstecken, Magie sichtbar machen.
...
**Lehre für Solarbot:** Die Begeisterung für den Optimizer zeigt, dass EVCCs Community **dringend nach Akku-Optimierung verlangt**. Die Idee verkauft sich — auch wenn das Produkt dahinter noch experimentell ist. Das ist exakt die Marktöffnung, die Solarbot bedienen muss. Siehe die ausführliche Optimizer-Analyse in Kapitel 5.
...
**Solarbot-Differenzierung:** Akku-Steuerung ist Solarbots **Kern**, nicht ein verzögertes Feature. Während EVCC den Akku als "autarken" Block behandelt, ist Solarbot von Tag eins darauf optimiert, ihn als orchestrierbaren Verbraucher zu behandeln. Die Botschaft: **"Was EVCC für deine Wallbox ist, ist Solarbot für deinen Akku."**
...
**Solarbot-Differenzierung:** Zeit-basierte Akku-Profile sind ein Standard-Feature, kein V2-Wunsch. Der Nutzer kann sagen: "Vor 6 Uhr morgens Akku voll, danach freigeben, ab 18 Uhr nur noch Hausverbrauch decken." Das ist im PRD bereits angedacht und sollte als USP markiert werden.
...
**Solarbot-Differenzierung:** EVCCs Default-Loop-Intervall ist 30 s, das ist für echte Nulleinspeisung zu langsam. Solarbot muss in der Lage sein, im Sekunden-Bereich zu regeln (mit den Sicherheitsmechanismen aus dem EEPROM-Patch). Die Botschaft: **"Solarbot reagiert so schnell, wie dein Smart Meter Daten liefert — ohne deinen Wechselrichter zu verschleißen."**
...
**Solarbot-Differenzierung:** Das Lizenzmodell ist eines der wichtigsten Kaufargumente. Solarbots Plan (Einmalkauf, Lizenz vor "Activate"-Button, keine Token-Erneuerung) ist exakt die Anti-These zum EVCC-Frust. Das muss prominent kommuniziert werden:
...
**Solarbot-Differenzierung:** Solarbot hat **kein YAML, das Nutzer editieren müssen**. Auto-Discovery, geführter Onboarding, sinnvolle Defaults. Die Botschaft: *"Wenn du in den letzten zehn Jahren keinen Online-YAML-Validator gebraucht hast, sollte sich das auch jetzt nicht ändern."*
...
**Solarbot-Differenzierung:** Solarbot ist im MVP bewusst auf 1 WR + 1 SM + 1 Akku begrenzt — aber V1.x sollte hier eine klare Priorisierungs-Engine bringen. Wichtig: Im Gegensatz zu EVCC, wo der Maintainer aktiv gegen Priorisierung argumentiert, ist Priorisierung für Solarbot ein **Kern-Konzept**, kein Streitthema. Damit kann Solarbot die Nutzer abholen, deren Wunsch von EVCC ignoriert wird.
...
**Solarbot-Differenzierung:** Solarbot **ist** für Balkonkraftwerke gebaut. Das ist nicht eine Ergänzung — das ist die Geburtsstunde. Dieses Detail hat strategische Bedeutung: Balkonkraftwerke sind das schnellst wachsende Segment im PV-Markt 2025/2026, und EVCC adressiert sie explizit nicht.
...
**Solarbot-Differenzierung:** Solarbot kommuniziert nie direkt mit Hardware. Damit gibt es keinen Modbus-Konflikt — niemals. Botschaft: *"Solarbot kommt nie in Konflikt mit deinen anderen Tools, weil Solarbot keine Hardware direkt anspricht."*
...
**Solarbot-Differenzierung:** Das ist eine wörtlich artikulierte Marktlücke, geschrieben von einem erfahrenen, technisch fähigen Nutzer in der EVCC-Community selbst. Solarbot ist die direkte Antwort auf genau diese Aussage. Das könnte fast 1:1 als Headline auf einer Landing Page funktionieren:

> **"EVCC ist großartig für dein Auto. Solarbot ist großartig für alles andere."**
...
Weil der Optimizer der einzige EVCC-Vorstoß direkt in Solarbots Kern-Territorium ist, verdient er eine eigene Tiefen-Analyse. Die Leitfrage: **Wie ernst müssen wir den EVCC Optimizer als Bedrohung für Solarbot nehmen?**

Kurze Antwort: **Ernst, aber die Bedrohung ist in den nächsten 12–18 Monaten noch nicht real — und die Architektur-Entscheidungen des Optimizers spielen Solarbot sogar in die Hände.**
...
### 5.7 Die vier Solarbot-Vorteile, die der Optimizer-Vergleich schärft
...
## 7. Solarbot-Differenzierung — wo wir konkret besser sind

| Dimension | EVCC | Solarbot |
|---|---|---|
...
> **"Solarbot läuft parallel zu EVCC. EVCC steuert dein Auto perfekt. Solarbot kümmert sich um den Rest — Akku, Heizstab, Wärmepumpe, Balkonkraftwerk."**
...
**2. Sie ist technisch ehrlich.** Solarbot ist objektiv schlechter beim Wallbox-Lastmanagement und beim Auto-spezifischen Plan. EVCC ist objektiv schlechter beim Akku und bei Balkonkraftwerken. Das ist keine Marketing-Pose, sondern Realität. Und der Optimizer-Stand bestätigt das öffentlich.

**3. Sie öffnet eine technische Schnittstelle.** Solarbot kann den Status von EVCC über die EVCC-API oder über HA-Entitäten lesen — und z. B. dem Akku sagen "Während EVCC das Auto lädt, halte den Akku-SoC". Das ist eine echte Integration, kein Feature-Klau.

**Konkrete Umsetzung:**
- Eine eigene Doku-Seite "Solarbot mit EVCC kombinieren" mit Setup-Anleitung.
- Ein "EVCC erkannt"-Hinweis im Solarbot-Onboarding mit der Frage "Soll Solarbot dein Auto-Laden EVCC überlassen?" und Default = Ja.
- Ein gemeinsamer Artikel auf alkly.de und ein Hinweis im EVCC-Discord/Forum.
- Ein fairer Blog-Post: *"Der EVCC Optimizer zeigt dir, was passieren könnte. Solarbot sorgt dafür, dass es passiert."*

---

## 9. Was Solarbot von EVCC lernen muss

Ehrlichkeit über die eigenen Stärken und Schwächen — und EVCC hat sechs Dinge richtig gemacht, die Solarbot kopieren oder übertreffen muss:

**1. Zuverlässige Hardware-Templates.** EVCCs Stärke ist die schiere Anzahl unterstützter Geräte. Solarbot muss das über HA-Entitäten lösen — und im Diagnose-Tab transparent zeigen, welche HA-Integrationen kompatibel sind.
...
**3. Aktive Community.** EVCC hat tausende GitHub-Discussions. Solarbot braucht von Tag eins einen aktiven Kanal (Discord ist gesetzt) und Alex muss persönlich präsent sein, mindestens in der Anfangszeit.

**4. Transparente Roadmap.** EVCC kommuniziert offen, was geplant ist und was nicht. Solarbot sollte mindestens einen öffentlichen Roadmap-Abschnitt auf alkly.de haben.

**5. "Magic Buttons".** Battery Boost ist ein einziger Klick mit komplexer Logik darunter. Solarbot braucht solche Knöpfe — z. B. "Akku jetzt voll", "Heute keine Einspeisung", "Wärmepumpe vorheizen".

**6. Migration-Erlebnis.** EVCC bekommt Nutzer von openWB ab, weil die Migration unkompliziert ist. Solarbot braucht eine analoge Migration vom eigenen Blueprint und von Hersteller-Apps — mit konkreten Schritt-für-Schritt-Anleitungen pro Quelle.

---

## 10. Was Solarbot besser machen muss als EVCC — die fünf konkreten Prioritäten

**Priorität 1: Akku als First-Class-Citizen.**
Während EVCC den Akku als autonome Black Box behandelt und der Optimizer nur Forecasts zeigt, muss Solarbot den Akku als steuerbaren Verbraucher mit Profilen, Zeitplänen und Priorisierung anbieten — und zwar **real regeln, nicht nur vorhersagen**. Das ist die direkte Antwort auf fünf offene EVCC-Discussions (#28341, #14739, #14648, #20208) und auf den aktuellen Stand des Optimizers.

**Priorität 2: Sekunden-Reaktionszeit mit EEPROM-Schutz.**
Solarbot muss schneller regeln als EVCCs 30-Sekunden-Loop, ohne Hardware zu verschleißen. Das ist die direkte Antwort auf Issue #22613.
...
> **"EVCC ist großartig für dein Auto. Solarbot ist großartig für alles andere — Akku, Heizstab, Wärmepumpe, Balkonkraftwerk."**
...
> **"Solarbot regelt deinen Akku in Sekunden — und schont deinen Wechselrichter, weil er nur dann schreibt, wenn es wirklich nötig ist."**

---

## 12. Risiken und blinde Flecken

**Risiko 1: EVCC implementiert echte Akku-Steuerung in den nächsten 12 Monaten.** Die Discussions #28341, #20208 und #14648 sind alt, aber der Druck wächst — und der Optimizer ist der erste echte Schritt in diese Richtung. Falls EVCC den Optimizer zur Produktionsreife bringt, schrumpft Solarbots Differenzierung in einem zentralen Punkt. **Mitigation:** Solarbot muss seinen Vorsprung nutzen, in dieser Zeit eine treue Userbase aufbauen und durch UX und Akku-Spezialisierung Tiefe gewinnen, die EVCC nicht so schnell aufholen kann. Der aktuelle Optimizer-Stand (nur Forecasts, kein Control) gibt uns mindestens 12–18 Monate.

**Risiko 2: Die Co-Existenz-Botschaft wird missverstanden.** Manche Nutzer könnten denken "Wenn ich EVCC habe, brauche ich Solarbot nicht" — auch wenn sie objektiv beide bräuchten. **Mitigation:** Eine sehr konkrete "Was Solarbot kann, was EVCC nicht kann"-Vergleichstabelle auf der Landing Page.

**Risiko 3: Die EVCC-Community ist groß und teils dogmatisch.** Eine zu aggressive Positionierung könnte als Angriff verstanden werden. **Mitigation:** Erstkontakt freundlich, mit echter Wertschätzung. Alex sollte selbst im EVCC-Discord auftauchen und Solarbot als komplementär vorstellen, bevor irgendwelche Marketing-Botschaften groß rausgehen.

**Risiko 4: Der Optimizer wird schneller produktionsreif als erwartet.** Das Team hinter EVCC ist fähig und motiviert. 36 Monate könnten 24 werden. **Mitigation:** Solarbot-Release darf nicht verzögert werden. Das 9-Wochen-MVP-Timeline ist genau richtig. HACS-Submission sollte früher als später kommen.
...
- Eine Entscheidung treffen: Will Alex Solarbot **explizit komplementär zu EVCC** positionieren oder neutral? Empfehlung: explizit komplementär.
...
**Vor Beta:**
- Die Co-Existenz-Doku schreiben ("Solarbot mit EVCC kombinieren"), inklusive konkreter Setup-Anleitung.
- Das "EVCC erkannt"-Onboarding-Detail im UX-Dokument verankern.
- Blog-Post-Entwurf vorbereiten: *"Der EVCC Optimizer zeigt dir, was passieren könnte. Solarbot sorgt dafür, dass es passiert."* — fair, anerkennend, mit klarer Positionierung.
- FAQ-Antwort vorbereiten: *"Wie unterscheidet sich Solarbot vom EVCC Optimizer?"* mit den vier Differenzierungs-Punkten aus 5.7.
...
**Post-Launch:**
- Ehrlicher Outreach in EVCC-GitHub-Discussions (#28341, #14648 etc.) mit dem Hinweis, dass Solarbot diese Use Cases bedient — ohne Spam, ohne Konfrontation, einfach als hilfreiche Information.
